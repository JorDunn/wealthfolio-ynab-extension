...
Scaffolding complete. Generator used: `@wealthfolio/addon-dev-tools create` with manual restructuring to exact handoff spec. All module files typed and AGPL-3.0-only headers applied.
...
- `vite.config.ts` with externalized host deps (react, react-dom, @wealthfolio/*, date-fns, lucide-react, recharts, @tanstack/react-query)
...
All src files carry SPDX-License-Identifier + copyright header.
...
npm run build               # Vite bundle → dist/addon.js
...
- `npm run build` → dist/addon.js exists, ~1 KB, minified
- `npm test -- --run` → 16 tests pass, exit 0
